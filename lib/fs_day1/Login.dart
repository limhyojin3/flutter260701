...
import 'package:flutter260629/fs_day1/SchedulePage.dart';
...
class _LoginPageState extends State<LoginPage> {
  ...
  Future<void> login() async {
    // 로그인 성공 or 실패 여부 판단
    // fs에서 가져온 아이디, 비밀번호 데이터랑 내가 입력한 데이터랑 같으면 로그인 성공!
    ...
    if(snap.docs.isEmpty){
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(content: Text("아이디/비밀번호 확인해라"))
      );
    } else{
      //로그인 성공
      Navigator.push(
        context,
        MaterialPageRoute(
            builder: (context) => SchedulePage(memberDocId: snap.docs.first.id),)
      );
    }
  }
